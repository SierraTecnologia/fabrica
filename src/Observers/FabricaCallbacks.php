...
class FabricaCallbacks
{
    ...
    public function handle($event, $payload)
    {
        ...
        // Ignore
        if (!app()->bound(FabricaService::class)) {
            return true;
        }
        ...
        if ($method == 'onBooting') {
            $model::addGlobalScope(new FabricaScope);
            return true;
        }

        if ($method == 'onCreating') {
            $business = app()->make(FabricaService::class);
